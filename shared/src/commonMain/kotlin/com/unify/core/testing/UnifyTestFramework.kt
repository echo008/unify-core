...
class UnifyTestFrameworkImpl : UnifyTestFramework {
    ...
    override suspend fun runSuite(suite: TestSuite): List<TestResult> {
        ...
        val results =
            if (suite.parallel && config.parallel) {
                // 并行执行
    ...
    private fun exportToHTML(report: TestReport): String {
        val successRate = UnifyPlatformUtils.formatDouble(report.successRate * 100, 2)
        val resultRows =
            report.results.joinToString("\n") { result ->
                val statusClass = result.status.name.lowercase()
                "<tr><td>${result.testName}</td><td class=\"$statusClass\">${result.status}</td>" +
                    "<td>${result.duration}ms</td><td>${result.message}</td></tr>"
            }

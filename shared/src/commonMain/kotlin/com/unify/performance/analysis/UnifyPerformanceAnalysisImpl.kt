...
// 使用统一的PerformanceMetrics定义，避免重复声明
...
class UnifyPerformanceAnalysisImpl : UnifyPerformanceAnalyzer {
    ...
    override fun detectIssues(metrics: PerformanceMetrics): List<PerformanceIssue> {
        ...
        if (metrics.memoryUsage.usagePercentage > HIGH_MEMORY_THRESHOLD) {
            issues.add(
                PerformanceIssue(
                    ...
                    severity = if (metrics.memoryUsage.usagePercentage > 95.0) IssueSeverity.CRITICAL else IssueSeverity.HIGH,
                    description = "内存使用率过高: ${metrics.memoryUsage.usagePercentage.toInt()}%",
        ...
        // 帧率检查
        if (metrics.fps < LOW_FRAME_RATE_THRESHOLD) {
            issues.add(
                PerformanceIssue(
                    type = IssueType.LOW_FRAME_RATE,
                    severity = if (metrics.fps < 15.0) IssueSeverity.CRITICAL else IssueSeverity.MEDIUM,
                    description = "帧率过低: ${metrics.fps.toInt()} FPS",
        ...
        // 电池电量检查 (替换不存在的storageIO属性)
        if (metrics.batteryLevel < LOW_BATTERY_THRESHOLD) {
            issues.add(
                PerformanceIssue(
                    type = IssueType.LOW_BATTERY,
                    severity = if (metrics.batteryLevel < 10.0) IssueSeverity.HIGH else IssueSeverity.MEDIUM,
                    description = "电池电量低: ${metrics.batteryLevel.toInt()}%",
    ...
    private fun calculateAverageMetrics(metrics: List<PerformanceMetrics>): PerformanceMetrics {
        if (metrics.isEmpty()) {
            return PerformanceMetrics(
                timestamp = getCurrentTimeMillis(),
                cpuUsage = 0.0,
                memoryUsage = MemoryUsage(0L, 0L, 0.0),
                fps = 0.0,
                networkLatency = 0L,
                batteryLevel = 0.0,
                thermalState = ThermalState.NORMAL,
        ...
        return PerformanceMetrics(
            ...
            memoryUsage =
                MemoryUsage(
                    used = metrics.map { it.memoryUsage.used }.average().toLong(),
                    total = metrics.map { it.memoryUsage.total }.average().toLong(),
                    usagePercentage = metrics.map { it.memoryUsage.usagePercentage }.average(),
                ),
            fps = metrics.map { it.fps }.average(),
            networkLatency = metrics.map { it.networkLatency }.average().toLong(),
            batteryLevel = metrics.map { it.batteryLevel }.average(),
            thermalState = ThermalState.NORMAL,
        )
    }
    ...
    private fun calculatePeakMetrics(metrics: List<PerformanceMetrics>): PerformanceMetrics {
        ...
        return PerformanceMetrics(
            ...
            memoryUsage =
                MemoryUsage(
                    used = metrics.maxOf { it.memoryUsage.used },
                    total = metrics.maxOf { it.memoryUsage.total },
                    usagePercentage = metrics.maxOf { it.memoryUsage.usagePercentage },
                ),
            fps = metrics.maxOf { it.fps },
    ...
    private fun generateRecommendations(
        ...
        // 基于平均指标生成通用建议
        if (averageMetrics.cpuUsage > 50.0) {
            recommendations.add("考虑使用更高效的数据结构")
        }

        if (averageMetrics.memoryUsage.usagePercentage > 60.0) {
            recommendations.add("定期进行内存清理")
        }

        if (averageMetrics.fps < 45.0) {

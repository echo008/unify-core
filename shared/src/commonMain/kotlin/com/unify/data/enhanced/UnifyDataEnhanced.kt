...
class UnifyDataEnhanced {
    ...
    suspend fun analyzeDataTrends(timeRange: TimeRange): DataTrendAnalysis {
        return try {
            ...
            val sentimentDistribution =
                relevantEntries
                    .groupingBy { it.sentiment }
                    .eachCount()

            val keywordFrequency =
                relevantEntries
                    .flatMap { it.keywords }
                    .groupingBy { it }
                    .eachCount()
    ...
    private fun matchesAdvancedCriteria(
        entry: SearchIndexEntry,
    ...
    private fun calculateAdvancedScore(
        entry: SearchIndexEntry,
        params: AdvancedSearchParams,
    ): Double {
    ...
    private fun updateDataMetrics(
        operation: DataOperation,
        dataSize: Int,
    ) {
        _dataMetrics.value =
            _dataMetrics.value.copy(

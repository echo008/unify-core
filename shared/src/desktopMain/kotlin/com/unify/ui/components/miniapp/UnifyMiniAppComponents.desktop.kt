...
object DesktopMiniAppUtils {
     ...
     * 启动原生应用
     */
    private fun launchNativeApp(
        appId: String,
        params: Map<String, String>,
    ) {
        try {
            val executablePath =
                params["executable"] ?: when (getOperatingSystem()) {
                    "windows" -> "app.exe"
                    "macos" -> "app.app"
                    "linux" -> "app"
                    else -> "app"
     ...
     * 启动跨平台应用
     */
    private fun launchCrossPlatformApp(
        appId: String,
        params: Map<String, String>,
    ) {
        try {
     ...
     * 启动通用桌面应用
     */
    private fun launchGenericDesktopApp(
        appId: String,
        params: Map<String, String>,
    ) {
        try {
    ...
    fun getSupportedApis(appId: String): List<String> {
        val baseApis =
            listOf(
                "desktop.fileSystem",
                "desktop.systemInfo",
                "desktop.processManagement",
            )

        val conditionalApis = mutableListOf<String>()

        if (Desktop.isDesktopSupported()) {
            conditionalApis.addAll(
                listOf(
                    "desktop.browse",
                    "desktop.mail",
                    "desktop.open",
     ...
     * 创建Windows快捷方式
     */
    private fun createWindowsShortcut(
        appName: String,
        executablePath: String,
    ): Boolean {
        return try {
     ...
     * 创建Linux桌面条目
     */
    private fun createLinuxDesktopEntry(
        appId: String,
        appName: String,
        executablePath: String,
    ): Boolean {
